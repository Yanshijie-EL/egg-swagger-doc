  ...
  for (let name of names) {
    ...
    if (stat.isFile() && path.extname(name) === '.js') {
      ...
      for (let i = 1; i < blocks.length; i++) {
        ...
        let direct = `${filepath.split('.js')[0].split('app/')[1]}`;
        // 解析路由
        ...
        if (routers) {
          ...
          path_method.operationId = `${direct.replace('/', '-')}-${func[i - 1]}`;
...
function generateAPIFunc(filepath) {
  ...
  func = Object.getOwnPropertyNames(instance).map(function(key) {
    return key;
  });
...
function isDeprecated(block) {
  return block.indexOf('@Deprecated') > -1;
}
