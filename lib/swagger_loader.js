      ...
      if (stat.isFile() && path.extname(name) === '.js') {
        ...
        for (let i = 1; i < blocks.length; i++) {
          ...
          if (routers) {
            ...
            path_method.operationId = `${routers[0][2].replace('/', '-')}-${i}`;
            ...
            // 解析description
            let descriptions = get_comments(blocks[i].replace(/^\s+\*\s+^/gm,'\n').replace(/^\s+\*\s*/gm,''), /@Description\s*\(?(?:[^@]*[^*\s\/)])?(?:\s|$|\))/gm);
            if (descriptions) {
              path_method.description = '';
              let m = 1;
            ...
            if (responses) {
              ...
              delete path_method.responses.default;
            }
