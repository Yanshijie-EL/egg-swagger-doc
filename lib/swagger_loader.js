...
module.exports = app => {
  ...
  for (let name of names) {
    ...
    if (stat.isFile() && path.extname(name) === '.js') {
      ...
      let controller = get_comments(blocks[0], is_win ? /@Controller.*\r\n/gm : /@Controller.*\n/gm)[0];
      ...
      for (let i = 1; i < blocks.length; i++) {
        ...
        // 解析summary
        let summarys = get_comments(blocks[i], is_win ? /@Summary.*\r\n/gm : /@Summary.*\n/gm);
        if (summarys) {
        ...
        // 解析description
        let descriptions = get_comments(blocks[i], is_win ? /@Description.*\r\n/gm : /@Description.*\n/gm);
        if (descriptions) {
          path_method.description = '';
          let i = 1;
        ...
        // 解析请求参数
        let requests = get_comments(blocks[i], is_win ? /@Request.*\r\n/gm : /@Request.*\n/gm);
        if (requests) {
        ...
        // 解析响应参数
        let responses = get_comments(blocks[i], is_win ? /@Response.*\r\n/gm : /@Response.*\n/gm);
        if (responses) {
        ...
        // 解析路由
        let routers = get_comments(blocks[i], is_win ? /@Router.*\r\n/gm : /@Router.*\n/gm);
        if (routers) {
