...
module.exports = app => {
  ...
  for (let name of names) {
    ...
    if (stat.isFile() && path.extname(name) === '.js') {
      ...
      let controller_regex = is_win ? /@Controller.*\r\n/gm :
        is_linux ? /@Controller.*\n/gm : /@Controller.*\r/gm;
      let controller = get_comments(blocks[0], controller_regex)[0];
      ...
      for (let i = 1; i < blocks.length; i++) {
        ...
        // 解析summary
        let summary_regex = is_win ? /@Summary.*\r\n/gm :
          is_linux ? /@Summary.*\n/gm : /@Summary.*\r/gm;
        let summarys = get_comments(blocks[i], summary_regex);
        if (summarys) {
        ...
        // 解析description
        let description_regex = is_win ? /@Description.*\r\n/gm :
          is_linux ? /@Description.*\n/gm : /@Description.*\r/gm;
        let descriptions = get_comments(blocks[i], description_regex);
        if (descriptions) {
          path_method.description = '';
          let i = 1;
        ...
        // 解析请求参数
        let request_regex = is_win ? /@Request.*\r\n/gm :
          is_linux ? /@Request.*\n/gm : /@Request.*\r/gm;
        let requests = get_comments(blocks[i], request_regex);
        if (requests) {
        ...
        // 解析响应参数
        let response_regex = is_win ? /@Response.*\r\n/gm :
          is_linux ? /@Response.*\n/gm : /@Response.*\r/gm;
        let responses = get_comments(blocks[i], response_regex);
        if (responses) {
        ...
        // 解析路由
        let router_regex = is_win ? /@Router.*\r\n/gm :
          is_linux ? /@Router.*\n/gm : /@Router.*\r/gm;
        let routers = get_comments(blocks[i], router_regex);
        if (routers) {
...
function is_linux() {
  return process.platform.toLowerCase.indexOf('linux') > -1;
}
